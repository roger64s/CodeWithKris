...
const stages: { key: Stage; label: string }[] = [
  ...
  { key: "assessment_and_matching", label: "Trial & match" }, { key: "delivery", label: "Deliver" },
...
export function ProjectOperations({ workspaceId, canEdit, preview, onOpenCoopEquity }: { workspaceId: string; canEdit: boolean; preview: boolean; onOpenCoopEquity: () => void }) {
  ...
  return <div className="operations-view">
    ...
    <div className="operations-grid">
      ...
      <section className="operations-panel"><header><div><span>Inclusive talent</span><h2>Human-reviewed matching</h2></div><Users size={18} /></header><div className="operations-metrics"><div><strong>{assessments.length}</strong><span>trial records</span></div><div><strong>{assessments.filter((item) => item.decision === "matched").length}</strong><span>matched</span></div><div><strong>{assessments.filter((item) => item.reviewed_at).length}</strong><span>human reviewed</span></div></div><div className="ai-governance"><Bot size={17} /><span>AI may organize practical artifacts and suggest support questions. A person reviews the evidence and approves every assignment.</span></div></section>
