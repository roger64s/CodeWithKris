...
export const CrmWorkspace: React.FC<CrmWorkspaceProps> = ({ view, isAuthenticated }) => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase.from("companies").select("id, co_name, co_url, co_type, co_lead_status, co_industry").order("co_name"),
      supabase.from("contacts").select("id, first_name, last_name, email, company_id, job_title, stage, lead_status").order("last_name"),
      ...
      if (companyResult.error || contactResult.error) {
        setStatus("CRM tables are not available yet. Apply supabase/crm_schema.sql to enable Company and Contact records.");
      }
    });
  }, [isAuthenticated]);
  ...
  const addCompany = async (event: React.FormEvent) => {
    ...
    }).select("id, co_name, co_url, co_type, co_lead_status, co_industry").single();
    if (error) {
    ...
    setCompanyName(""); setCompanyUrl(""); setIndustry("");
  ...
  const addContact = async (event: React.FormEvent) => {
    ...
    }).select("id, first_name, last_name, email, company_id, job_title, stage, lead_status").single();
    if (error) {
      setStatus(`Contact was not saved: ${error.message}`);
      return;
    }
    setContacts((current) => [...current, data as ContactRecord]);
    setFirstName(""); setLastName(""); setEmail(""); setCompanyId(""); setJobTitle("");
  ...
  if (view === "companies") {
    return <div className="financial-content">
      <div className="admin-section contribution-ledger">
        ...
        <div className="table-scroll"><table className="coop-table"><thead><tr><th>Company</th><th>Type</th><th>Status</th><th>Industry</th></tr></thead><tbody>
          {companies.length ? companies.map((company) => <tr key={company.id}><td><strong>{company.co_name}</strong><small>{company.co_url || "No website recorded"}</small></td><td>{company.co_type || "Not classified"}</td><td>{company.co_lead_status || "Not set"}</td><td>{company.co_industry || "Not set"}</td></tr>) : <tr><td colSpan={4}>No accessible company records yet.</td></tr>}
        </tbody></table></div>
      </div>
  ...
  return <div className="financial-content">
    <div className="admin-section contribution-ledger">
      ...
      <div className="table-scroll"><table className="coop-table"><thead><tr><th>Contact</th><th>Company</th><th>Job title</th><th>Stage</th><th>Lead status</th></tr></thead><tbody>
        {contacts.length ? contacts.map((contact) => <tr key={contact.id}><td><strong>{contact.first_name} {contact.last_name}</strong><small>{contact.email || "No email recorded"}</small></td><td>{companies.find((company) => company.id === contact.company_id)?.co_name || "Unassigned"}</td><td>{contact.job_title || "Not set"}</td><td>{contact.stage || "Not set"}</td><td>{contact.lead_status || "Not set"}</td></tr>) : <tr><td colSpan={5}>No accessible contact records yet.</td></tr>}
      </tbody></table></div>
    </div>
    <form className="admin-section contribution-form entry-panel" onSubmit={addContact}>
      ...
      <label>Company<select value={companyId} onChange={(event) => setCompanyId(event.target.value)}><option value="">Unassigned</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.co_name}</option>)}</select></label>
