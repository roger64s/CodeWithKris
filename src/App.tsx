...
type Screen = 'register' | 'signin' | 'templates' | 'record' | 'practice' | 'progress' | 'dictionary'
...
function App() {
  ...
  </div><nav className="bottom-nav" aria-label="Main navigation">{[['templates', 'Templates', '▦'], ['record', 'Record', '●'], ['practice', 'Practice', '◌'], ['progress', 'Progress', '▥']].map(([value, label, icon]) => <button className={screen === value ? 'active' : ''} key={value} onClick={() => navigate(value as Screen)}><span aria-hidden="true">{icon}</span>{label}</button>)}</nav></main>
